...
import { RefreshCw, ExternalLink, AlertTriangle, Search, MousePointerClick, Bell, CalendarClock, Download, Timer } from 'lucide-react'

const AUTO_REFRESH_SECONDS = 300 // 5 minutos

export default function DealsList() {
  const [deals, setDeals] = useState(() => {
    try { return JSON.parse(localStorage.getItem('deals_cache') || '[]') } catch { return [] }
  })
  ...
  const [lastUpdated, setLastUpdated] = useState(() => localStorage.getItem('deals_cache_time') || null)
  ...
  function exportCSV() {
    const headers = ['Negócio', 'Empresa', 'Responsável', 'Email', 'Etapa', 'Funil', 'Parado há (dias)', 'Última atualização', 'Tipo', 'Link']
  ...
  const owners = [...new Set(deals.map(d => d.ownerName).filter(Boolean))].sort()

  const filtered = deals
    .filter(d => {
      if (search && !d.title?.toLowerCase().includes(search.toLowerCase()) &&
          !d.ownerName?.toLowerCase().includes(search.toLowerCase()) &&
          !d.organization?.toLowerCase().includes(search.toLowerCase())) return false
    ...
    <div className="space-y-4">
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        ...
        {loading ? (
          ...
          <div className="py-12 text-center text-gray-400 text-sm">
            {deals.length === 0
              ? <span>Clique em <strong>Atualizar</strong> para carregar os dados</span>
          ...
          <>
            <div className="text-xs text-gray-500 mb-3 flex flex-wrap items-center gap-3">
              <span className="font-medium">{filtered.length} negócio(s) encontrado(s)</span>
              {futureTasks > 0 && (
                <span className="inline-flex items-center gap-1 text-blue-600 bg-blue-50 border border-blue-200 px-2 py-0.5 rounded-full">
                  <CalendarClock size={11} /> {futureTasks} com tarefa agendada (sem notificação)
                </span>
              )}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-xs text-gray-500 border-b border-gray-100">
                    ...
                    <th className="text-left py-2 pr-4 font-medium">Última atualização</th>
                    <th className="text-left py-2 pr-4 font-medium">Notificação</th>
                    <th />
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {filtered.map(deal => (
                    <DealRow key={deal.id} deal={deal} staleDays={staleDays} notified={notifiedMap[deal.id]} />
                  ))}
                </tbody>
              </table>
            </div>
    ...
    <tr className="hover:bg-gray-50 transition-colors">
      ...
      <td className="py-3 pr-4">
        ...
        <span className={`inline-block mt-0.5 text-xs px-1.5 py-0.5 rounded-full font-medium ${
          deal.dealType === 'Lead' ? 'bg-blue-50 text-blue-600' : 'bg-green-50 text-green-700'
        }`}>
      ...
      <td className="py-3 pr-4">
        ...
        {deal.ownerEmail && <div className="text-xs text-gray-400">{deal.ownerEmail}</div>}
        {deal.authorName && deal.authorName !== deal.ownerName && (
          <div className="text-xs text-gray-400 mt-0.5">
            <span className="text-gray-300">criador: </span>{deal.authorName}
          </div>
        )}
      </td>
      ...
      <td className="py-3">
        <a href={deal.webUrl} target="_blank" rel="noopener noreferrer"
          className="text-blue-500 hover:text-blue-700 transition-colors" title="Abrir no Agendor">
