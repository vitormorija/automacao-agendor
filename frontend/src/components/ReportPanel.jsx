...
import { RefreshCw, TrendingUp, Users, Tag, GitBranch, Clock, AlertTriangle } from 'lucide-react'
import {
  BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer,
  PieChart, Pie, Cell, Legend, LineChart, Line,
} from 'recharts'

const COLORS = ['#1a56db', '#0ea5e9', '#8b5cf6', '#f59e0b', '#10b981', '#ef4444', '#ec4899', '#64748b']
...
export default function ReportPanel() {
  const [data, setData] = useState(() => {
    try { return JSON.parse(localStorage.getItem('report_cache') || 'null') } catch { return null }
  })
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState(null)
  const [typeFilter, setTypeFilter] = useState('todos')
  const [ownerFilter, setOwnerFilter] = useState('todos')
  const [lastUpdated, setLastUpdated] = useState(() => localStorage.getItem('report_cache_time') || null)
  ...
  if (!data && loading) return (
    <div className="flex flex-col items-center justify-center py-20 text-gray-400">
      ...
      <span className="text-sm">Carregando relatório... (pode levar alguns segundos)</span>
    </div>
  )
  ...
  if (!data) return (
    <div className="flex flex-col items-center justify-center py-20 text-gray-400 gap-4">
      ...
      <div className="text-center">
        <p className="text-sm font-medium text-gray-600">Nenhum dado carregado ainda</p>
        <p className="text-xs text-gray-400 mt-1">Clique em "Carregar relatório" para buscar os dados</p>
      </div>
  ...
  const owners = [...new Set(allDeals.map(d => d.ownerName).filter(Boolean))].sort()
  ...
  const buildCategoryData = (deals) => {
    const map = {}
    for (const d of deals) { const c = d.orgCategory || 'Indefinido'; map[c] = (map[c] || 0) + 1 }
    return Object.entries(map).sort((a, b) => b[1] - a[1]).map(([name, count]) => ({ name, count }))
  }

  const buildFunnelData = (deals) => {
    const map = {}
    for (const d of deals) { const f = d.funnel || 'Sem funil'; map[f] = (map[f] || 0) + 1 }
    return Object.entries(map).sort((a, b) => b[1] - a[1]).map(([name, count]) => ({ name, count }))
  }

  const buildUrgencyData = (deals) => [
    { label: '15–20 dias', count: deals.filter(d => d.daysSinceUpdate >= 15 && d.daysSinceUpdate <= 20).length },
    { label: '21–30 dias', count: deals.filter(d => d.daysSinceUpdate >= 21 && d.daysSinceUpdate <= 30).length },
    { label: '31–45 dias', count: deals.filter(d => d.daysSinceUpdate >= 31 && d.daysSinceUpdate <= 45).length },
    { label: '46–60 dias', count: deals.filter(d => d.daysSinceUpdate >= 46 && d.daysSinceUpdate <= 60).length },
    { label: '60+ dias',   count: deals.filter(d => d.daysSinceUpdate > 60).length },
  ]
  ...
  const summary = {
    ...
    avgDays: filteredDeals.length ? Math.round(totalDays / filteredDeals.length) : 0,
    maxDays: filteredDeals.length ? Math.max(...filteredDeals.map(d => d.daysSinceUpdate)) : 0,
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-gray-800">Relatório de Negócios Parados</h2>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <div className="flex rounded-lg border border-gray-200 overflow-hidden text-sm">
            {[['todos','Todos'], ['Lead','🔵 Lead'], ['Negócio','🟢 Negócio']].map(([val, label]) => (
              <button key={val} onClick={() => setTypeFilter(val)}
                ...
                {label}
                {val === 'Lead' && <span className="ml-1 text-xs opacity-75">({summary.leads})</span>}
                {val === 'Negócio' && <span className="ml-1 text-xs opacity-75">({summary.negocios})</span>}
              </button>
            ))}
          </div>
          {/* Filtro responsável */}
      ...
      >
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={ownerData} layout="vertical" margin={{ left: 10, right: 30 }}>
            ...
            <Tooltip
              formatter={(val, _, props) => [`${val} negócio(s) • média ${props.payload.avgDays}d`, 'Parados']}
              contentStyle={{ fontSize: 13 }}
            />
            <Bar dataKey="count" fill="#1a56db" radius={[0, 4, 4, 0]} label={{ position: 'right', fontSize: 12 }} />
          </BarChart>
        </ResponsiveContainer>
      </ChartCard>
        ...
        >
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={urgencyData} margin={{ top: 8 }}>
              ...
              <Tooltip formatter={val => [`${val} negócio(s)`, 'Parados']} contentStyle={{ fontSize: 13 }} />
            ...
            <PieChart>
              ...
              <Legend
                ...
                formatter={(value, entry) => (
                  <span style={{ fontSize: 12 }}>{value} ({entry.payload.count})</span>
                )}
              />
              <Tooltip formatter={val => [`${val} negócio(s)`, '']} contentStyle={{ fontSize: 13 }} />
            </PieChart>
          </ResponsiveContainer>
        </ChartCard>
      </div>
        ...
        {weeklyHistory.length > 0 ? (
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={weeklyHistory} margin={{ top: 8 }}>
              ...
              <Tooltip
                formatter={(val, key) => [
                  key === 'total_stale' ? `${val} negócio(s)` : `${Math.round(val)} dias`,
                  key === 'total_stale' ? 'Total parados' : 'Média de dias'
                ]}
                contentStyle={{ fontSize: 13 }}
              />
              <Legend formatter={v => v === 'total_stale' ? 'Total parados' : 'Média de dias'} />
              <Line type="monotone" dataKey="total_stale" stroke="#1a56db" strokeWidth={2} dot={{ r: 4 }} />
              <Line type="monotone" dataKey="avg_days" stroke="#f59e0b" strokeWidth={2} dot={{ r: 4 }} strokeDasharray="5 5" />
            </LineChart>
          </ResponsiveContainer>
        ) : (
          <div className="flex items-center justify-center h-32 text-gray-400 text-sm">
            O gráfico de histórico será preenchido automaticamente toda sexta-feira às 11h.
          </div>
        )}
      </ChartCard>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-xs text-gray-500 border-b border-gray-100">
                ...
                <th className="text-left py-2 pr-3 font-medium">Empresa / Categoria</th>
                <th className="text-left py-2 pr-3 font-medium">Responsável</th>
                <th className="text-left py-2 pr-3 font-medium">Funil / Etapa</th>
                <th className="text-center py-2 font-medium">Parado há</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {[...data.deals].sort((a, b) => b.daysSinceUpdate - a.daysSinceUpdate).map(deal => (
                <tr key={deal.id} className="hover:bg-gray-50">
                  <td className="py-2.5 pr-3">
                    <a href={deal.webUrl} target="_blank" rel="noopener noreferrer"
                      className="font-medium text-blue-600 hover:underline max-w-xs truncate block">
                      {deal.title}
                    </a>
                  </td>
                  <td className="py-2.5 pr-3">
                    <div className="text-gray-700 text-xs">{deal.organization || '—'}</div>
                    {deal.orgCategory && (
                      <span className={`text-xs px-1.5 py-0.5 rounded-full ${deal.orgCategory === 'Lead' ? 'bg-blue-50 text-blue-600' : 'bg-gray-100 text-gray-500'}`}>
                        {deal.orgCategory}
                      </span>
                    )}
                  </td>
                  <td className="py-2.5 pr-3 text-gray-700 text-xs">{deal.ownerName || '—'}</td>
                  <td className="py-2.5 pr-3">
                    <div className="text-gray-500 text-xs">{deal.funnel || '—'}</div>
                    <div className="text-gray-400 text-xs">{deal.stage || ''}</div>
                  </td>
                  <td className="py-2.5 text-center">
                    <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-semibold ${
                      deal.daysSinceUpdate >= 45 ? 'bg-red-100 text-red-700' :
                      deal.daysSinceUpdate >= 30 ? 'bg-orange-100 text-orange-700' :
