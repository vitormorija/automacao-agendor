  ...
  async function fetchConfig() {
    ...
    try {
      ...
      const preset = CRON_PRESETS.find(
        (p) => p.value === d.cron_schedule && p.value !== 'custom',
      );
  ...
  return (
    <div className="space-y-5 max-w-2xl">
      ...
      <Section title="Regras de monitoramento">
        ...
        <Field label="Notificações ativas">
          <label className="flex items-center gap-2 cursor-pointer">
            <div
              onClick={() =>
                handleChange(
                  ...
                  config.notifications_enabled === 'true' ? 'false' : 'true',
                )
              }
            ...
            >
              <span
                className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${config.notifications_enabled === 'true' ? 'translate-x-6' : 'translate-x-1'}`}
              />
            </div>
        ...
        <Field
          ...
          help="Define quando a verificação automática será executada"
        >
      ...
      <Section title="Administradores">
        ...
        <Field
          label="Notificar o criador do card"
          help="Se ativado, quem criou o negócio também recebe o email (além do responsável)"
        >
          <label className="flex items-center gap-2 cursor-pointer">
            <div
              onClick={() =>
                handleChange(
                  'notify_author',
                  config.notify_author === 'true' ? 'false' : 'true',
                )
              }
            ...
            >
              <span
                className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${config.notify_author !== 'false' ? 'translate-x-6' : 'translate-x-1'}`}
              />
            </div>
            <span className="text-sm text-gray-700">
