...
import { CheckCircle, XCircle, RefreshCw, Clock, MousePointerClick } from 'lucide-react'

const PAGE_SIZE = 20

function ResolvedSection() {
  const [data, setData] = useState(() => {
    try { return JSON.parse(localStorage.getItem('resolved_cache')) || null } catch { return null }
  })
  ...
  const list = data ? (activeTab === 'resolved' ? data.resolved : data.pending) : []

  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5 mb-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-base font-semibold text-gray-800">Resolução de notificações</h2>
          <p className="text-sm text-gray-500 mt-0.5">Cards notificados que foram atualizados no Agendor</p>
        </div>
        <button
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-3 mb-4">
            <div className="bg-green-50 border border-green-100 rounded-lg p-3 text-center">
              <div className="text-2xl font-bold text-green-700">{data.resolvedCount}</div>
              <div className="text-xs text-green-600 mt-0.5">Resolvidos</div>
            </div>
            <div className="bg-amber-50 border border-amber-100 rounded-lg p-3 text-center">
              <div className="text-2xl font-bold text-amber-700">{data.pendingCount}</div>
              <div className="text-xs text-amber-600 mt-0.5">Aguardando</div>
            </div>
            <div className="bg-blue-50 border border-blue-100 rounded-lg p-3 text-center">
              <div className="text-2xl font-bold text-blue-700">{data.resolvedRate}%</div>
              <div className="text-xs text-blue-600 mt-0.5">Taxa de resolucao</div>
            </div>
          </div>
          ...
          <div className="flex gap-2 mb-3 border-b border-gray-100">
            ...
            >
              Resolvidos {data.resolvedCount > 0 && <span className="ml-1 bg-green-100 text-green-700 text-xs px-1.5 py-0.5 rounded-full">{data.resolvedCount}</span>}
            </button>
            <button
            ...
            >
              Aguardando {data.pendingCount > 0 && <span className="ml-1 bg-amber-100 text-amber-700 text-xs px-1.5 py-0.5 rounded-full">{data.pendingCount}</span>}
            </button>
          </div>

          {list.length === 0 ? (
            <div className="py-6 text-center text-gray-400 text-sm">
              {activeTab === 'resolved' ? 'Nenhum deal resolvido ainda.' : 'Nenhum deal aguardando resolucao.'}
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-xs text-gray-500 border-b border-gray-100">
                    ...
                    <th className="text-left py-2 pr-4 font-medium">Notificado em</th>
                    <th className="text-left py-2 font-medium">
                      {activeTab === 'resolved' ? 'Resolvido em' : 'Aguardando desde'}
                    </th>
                  </tr>
                </thead>
                    ...
                    <tr key={d.deal_id} className="hover:bg-gray-50">
                      ...
                      <td className="py-3 pr-4">
                        <div className="font-medium text-gray-800 max-w-xs truncate">
                          {d.web_url
                            ? <a href={d.web_url} target="_blank" rel="noopener noreferrer" className="hover:text-blue-600 hover:underline">{d.deal_title}</a>
                            : d.deal_title}
                        </div>
                        {d.deal_type && (
                      ...
                      <td className="py-3 pr-4 text-gray-500 text-xs whitespace-nowrap">
                        ...
                        {d.notification_count > 1 && (
                          <div className="text-gray-400">{d.notification_count}x notificado</div>
                        )}
                      </td>
                      <td className="py-3 text-xs whitespace-nowrap">
                        {d.resolved && d.resolved_at
                          ? <span className="text-green-600">{new Date(d.resolved_at).toLocaleString('pt-BR')}</span>
                          : <span className="text-amber-500">{new Date(d.last_notified_at).toLocaleString('pt-BR')}</span>}
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-xs text-gray-500 border-b border-gray-100">
                    ...
                    <th className="text-left py-2 pr-4 font-medium">Link clicado</th>
                      ...
                      <td className="py-3 pr-4">
                        {log.clicked_at ? (
                          <div className="flex items-center gap-1 text-green-600 text-xs">
                            ...
                            <span>{new Date(log.clicked_at).toLocaleString('pt-BR', { day:'2-digit', month:'2-digit', hour:'2-digit', minute:'2-digit' })}</span>
                          </div>
                        ) : (
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between mt-4 text-sm">
                ...
                <span className="text-gray-500">Pagina {page + 1} de {totalPages}</span>
                <button
