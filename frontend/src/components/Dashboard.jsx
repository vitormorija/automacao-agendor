...
import { AlertTriangle, RefreshCw, Clock, CheckCircle, XCircle, Play, Search, Mail, MousePointerClick, Users, TrendingUp } from 'lucide-react'
...
export default function Dashboard({ onTabChange }) {
  ...
  const [checkResult, setCheckResult] = useState(() => {
    try {
      const cached = JSON.parse(localStorage.getItem('dashboard_check_cache') || 'null')
  ...
  async function checkOnly() {
    ...
    try {
      ...
      toast.success(`${result.total} negócio(s) parado(s) encontrado(s)`, { id: toastId, duration: 4000 })
    } catch {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          value={status?.totalClicked != null ? `${status.totalClicked} (${status.clickRate}%)` : '—'}
          bg="bg-purple-50"
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => { fetchStatus(); fetchLogs() }}
            className="text-gray-400 hover:text-gray-600 transition-colors" title="Atualizar">
            <RefreshCw size={16} />
          </button>
        </div>
      ...
      {/* Botões de ação */}
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <h2 className="text-base font-semibold text-gray-800 mb-1">Controle manual</h2>
        <p className="text-sm text-gray-500 mb-4">
          Verifique primeiro quais negócios estão parados, e só depois envie as notificações se quiser.
        </p>
        <div className="flex flex-wrap gap-3">
          <button onClick={checkOnly} disabled={running || sending}
            ...
            {running ? <RefreshCw size={16} className="animate-spin" /> : <Search size={16} />}
            {running ? 'Verificando...' : 'Verificar negócios parados'}
          </button>
          {checkResult && (
            <button onClick={sendNow} disabled={sending || running}
              ...
              {sending ? 'Enviando...' : `Enviar notificações (${checkResult.total} negócios)`}
            </button>
          )}
        </div>
          ...
          <div className="mt-4 bg-amber-50 border border-amber-200 rounded-lg p-3">
            ...
            <div className="space-y-1 max-h-48 overflow-y-auto">
              {checkResult.deals?.map(deal => (
                <div key={deal.id} className="text-xs text-amber-700 flex items-center justify-between gap-2">
                  <span className="truncate">{deal.title}</span>
                  <span className="shrink-0 font-semibold">{deal.daysSinceUpdate}d · {deal.ownerName || '—'}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {checkResult?.deals?.length > 0 && (
        <OwnerRanking deals={checkResult.deals} onViewAll={() => onTabChange('deals')} />
      )}
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => onTabChange('history')} className="text-sm text-blue-600 hover:underline">
            Ver todas →
          </button>
        </div>
        {loadingLogs ? (
          ...
          <div className="divide-y divide-gray-100">
            {logs.map(log => <NotificationRow key={log.id} log={log} />)}
          </div>
        )}
      </div>
    </div>
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-semibold text-gray-800">Negócios parados por responsável</h2>
        </div>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="shrink-0 ml-2 flex items-center gap-2 text-xs text-gray-500">
                  ...
                  <span>· máx <span className="font-medium text-amber-600">{data.maxDays}d</span></span>
                </div>
              </div>
      ...
      <div className="mt-4 pt-4 border-t border-gray-100 grid grid-cols-3 gap-4 text-center">
        ...
        <div>
          <div className="text-xl font-bold text-amber-600">
            {Math.round(deals.reduce((s, d) => s + d.daysSinceUpdate, 0) / deals.length)}d
          </div>
