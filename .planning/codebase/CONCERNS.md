...
**Repo root cluttered with non-application files — RESOLVIDO em 2026-08-10:**
- Era: `package.json`/`package-lock.json` rastreados na raiz declarando só `pptxgenjs`, ao lado de scripts avulsos (`make_*.js`, `capture_screenshots.py`) e de artefatos gerados (`Agendor_Atualizar_Cards.pptx` ~29,7MB, `unpacked/` com 28MB, planilhas e relatórios).
- Correção: duas pastas gitignoradas passaram a concentrar tudo — `tools/` (scripts de geração + manifesto próprio) e `arquivos/` (`apresentacoes/`, `documentos/`, `dados/`). Nada foi apagado: os arquivos não estavam versionados, então remoção seria irreversível; foram movidos. Os manifestos foram destrastreados (`git rm --cached`), porque os scripts que eles servem nunca estiveram no git e o manifesto sozinho não tinha o que executar. O `.gitignore` passou a ignorar por PASTA, e não por arquivo: a lista anterior nomeava cada artefato, então todo arquivo novo do mesmo tipo nascia versionável até alguém lembrar de acrescentá-lo.
- Correção adjacente: `pptxgenjs` também estava em `backend/devDependencies` sem nenhum importador — peso morto em todo `npm ci` do job de backend do CI, e superfície de cadeia de suprimentos desnecessária num projeto sob auditoria de dependência. Removido; suíte segue verde e o audit continua em zero.
- Ressalva medida: o impacto "bloat every clone" registrado antes NÃO se confirmava — só o par `package.json`/`package-lock.json` estava rastreado; os binários já eram ignorados. O ganho real foi de navegabilidade e de árvore de dependências, não de tamanho de clone.
