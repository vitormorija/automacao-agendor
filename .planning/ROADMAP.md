...
**Plans**: 7 planos originais + 4 de gap closure (r1) + 7 de gap closure (r2) + 9 de gap closure (r3) + 7 de gap closure (r4) + 1 (promoção do todo `in3-08` ao plano 04-35) + 3 de gap closure (r5) = 38 (execução estritamente sequencial; `parallelization: false`)
...
**Gap closure r5** (fonte: `04-REVIEW-r5.md`, `round: 5`, status `issues_found` — 1 critical, 5 warning, 4 info; a rodada 5 verificou ceticamente as conclusões da r4 e REABRIU a fase pela quinta vez). 3 planos **aditivos**, waves 29-31, `gap_closure: true`. **Escopo travado pelo usuário (2026-08-05):** corrigir **somente** CR5-01 e WR5-01, **reescrever** o todo `cr4-01b` — que documentava a causa errada e cuja "correção proposta" fecharia o item com o defeito de pé — e registrar os **oito** achados restantes (WR5-02..WR5-05 e IN5-01..IN5-04) como **todos pendentes**, no mesmo precedente de IN-01..IN-04 (04-09), IN2-01..IN2-04 (04-18), IN3-01..IN3-08 (04-27) e IN4-02..IN4-06 (04-34). **A MUDANÇA DE PERFIL desta rodada é o que a distingue das quatro anteriores:** os achados **não mudam quem recebe** — são defeitos nos **instrumentos**, e quebram os sinais construídos para perceber falha (a chave que colide com o contrato do lock e produz um toast vermelho em branco no disparo manual; o alarme de supressão total desarmado por qualquer guarda anterior que faça `continue`; o oráculo de quem-recebe que deixou de medir o e-mail duplicado). O eixo "quem recebe" está estável desde o 04-21. **Requisito estrutural desta rodada — o INVENTÁRIO DE IRMÃOS da r4 é MANTIDO, acrescido de duas cláusulas:** o revisor verificou que o mandato anterior **funcionou e não virou cerimônia** (todo item marcado `verificada-e-sã` que ele amostrou carrega grep ou caso de teste, e vários **executam** o irmão em vez de lê-lo — melhora real sobre as rodadas 1-3), mas mediu **três vezes nesta mesma rodada** o seu ponto cego estrutural: o inventário pergunta *"este irmão está são?"* e nunca *"o que eu acabei de construir DEPENDE do comportamento dele?"*. O ponto cego nasceu de o 04-28 ter classificado a dedup do dia como sã **com evidência correta para a pergunta errada** — e é o `continue` dessa mesma dedup que desarma o alarme que o 04-28 estava construindo. As duas cláusulas novas: **(a) direção reversa** — listar por escrito de que comportamento alheio o conserto novo depende, isto é, o que pode **neutralizá-lo** (quais guardas anteriores impedem um contador agregado de alcançar o denominador; o que pode calar um alarme), e não apenas se os irmãos estão sãos; e **(b) retroatividade da justificativa** — toda justificativa escrita num comentário novo vira **grep obrigatório** no mesmo arquivo e na mesma função por construções que ela condena (essa cláusula sozinha teria pego WR5-02 em menos de um minuto):
...
- [x] 04-37-PLAN.md — O alarme de supressão total sobrevive a um `continue` anterior: o numerador passa a ser contado no topo do laço, a mensagem afirma sobre a rodada e o todo `cr4-01b` é reescrito pelo mecanismo (WR5-01)
- [x] 04-38-PLAN.md — WR5-02..WR5-05 e IN5-01..IN5-04 como todos pendentes, e a gap closure r5 registrada no ROADMAP
