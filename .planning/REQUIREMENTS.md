...
- [x] **REL-04**: O estado de categorias de organização é isolado por execução de `getStaleDeals` — nenhuma execução lê, apaga, reutiliza ou contamina o estado de outra, e categoria obsoleta nunca é usada entre execuções (redação atualizada por decisão C9, 2026-08-05: a formulação original pedia "TTL/invalidação do `orgCategoryCache`", um mecanismo que deixou de existir no 04-12 — o cache virou um `Map` criado dentro da própria execução; a remoção da limpeza está aprovada pelo usuário e não é regressão)
...
- [x] **REL-06**: Falha na consulta de tarefas futuras aborta a rodada sem disparar notificações — resultado completo ou falha explícita, nunca proteção parcial silenciosa (derivado de DESC-2 / Decisão Q2, 04-DELIVERY-CONTRACT.md, 2026-08-04; a mesma regra passou a valer para a **consulta de categoria da organização** a partir de CR3-01, 2026-08-05 — até então ela falhava ABERTA, e um erro transitório fazia uma organização de categoria excluída ser notificada em silêncio. Rota aprovada pelo usuário: a consulta entra na política única de retry da borda e, se ainda assim falhar, o negócio é tratado como **indecidível** — fora de todo e-mail dirigido ao responsável, mas visível no painel, no consolidado do admin e no snapshot, sem abortar a rodada. Continua sendo "resultado completo ou falha explícita": aqui a falha é explícita e escopada ao negócio afetado, em vez de custar a rodada inteira)
