...
- **Humanas (2026-08-04, supersedem onde conflitam):** Q1 (04-06 corrige DESC-1), Q2 (04-02 fail-safe para DESC-2, plano próprio), Q3 (`getDealById` em `agendor.js`; **não** exportar a instância bruta), Q4 (`auto_advance` OFF na fase; C1-C6 obrigatórios), Q5 (verificação retroativa da Fase 3 = gate de transição, após 04-VERIFICATION e antes do planejamento da Fase 5), npm audit informativo com critério restrito ao escopo corrigido, **Q6** (checkpoint C3+C4 do 04-05 aprovado; nodemailer mantido em 9.0.4; nova semântica de fallback por endereço DNS **aceita** nesta fase — `connectionTimeout` vale por endereço A/AAAA, pior caso vira `30N + 69`s por destinatário com N=2 observado, a garantia de "~1min40s" de D-02 deixa de ser invariável, e o estudo de deadline global fica como pendência pré-go-live em `todos/pending/rel-02b-deadline-global-smtp.md`; ao implementá-lo, `Promise.race` ingênuo é proibido sem encerrar o transporte SMTP subjacente).
